...
export default function FieldTeamPage({ marketingEmbedded = false }: { marketingEmbedded?: boolean }) {
  ...
  return <section className="grid gap-4" data-testid="field-team-workspace">
    ...
    {mode === "live" ? <>
      ...
      {selected ? <EmployeeDetail location={selected} onActivity={() => navigate(`/app/employees?view=activity&employee=${selected.userId}`)} onHistory={() => setMode("history")} /> : null}
    </> : <>
      <div className="grid gap-3 rounded-md border border-blue-200 bg-blue-50/40 p-4 shadow-sm md:grid-cols-[minmax(260px,1fr)_220px_auto] md:items-end"><EmployeePicker employees={data.employees} value={selectedId ?? ""} onChange={selectEmployeeId} allowAll={false} /><label><span className={labelClass}>Activity Date</span><input className={inputClass} type="date" max={businessDate()} value={historyDate} onChange={(event) => setHistoryDate(event.target.value)} /></label><Button icon={<Users className="h-4 w-4" />} onClick={() => navigate(`/app/employees?view=activity&employee=${selectedId}`)} disabled={!selectedId}>Activity & Reports</Button></div>
