...
export default function FieldTeamPage({ marketingEmbedded = false }: { marketingEmbedded?: boolean }) {
  ...
  if (currentQuery.isLoading) return <LoadingBlock label="Loading your field team feed" />;
  ...
  return <section className="grid gap-4" data-testid="field-team-workspace">
    ...
    {mode === "live" ? <>
      ...
      {selected ? <EmployeeDetail location={selected} onActivity={() => navigate(`/app/employees?view=activity&employee=${selected.userId}`)} onHistory={() => setMode("history")} onReport={() => navigate(`/app/reports?view=marketing&employee=${selected.userId}&preset=month`)} /> : null}
    </> : <>
      <div className="grid gap-3 rounded-md border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[minmax(260px,1fr)_220px_auto] md:items-end"><EmployeePicker employees={data.employees} value={selectedId ?? ""} onChange={selectEmployeeId} allowAll={false} /><label><span className={labelClass}>Activity Date</span><input className={inputClass} type="date" max={businessDate()} value={historyDate} onChange={(event) => setHistoryDate(event.target.value)} /></label><Button icon={<Users className="h-4 w-4" />} onClick={() => navigate(`/app/employees?view=activity&employee=${selectedId}`)} disabled={!selectedId}>Activity & Performance</Button></div>
