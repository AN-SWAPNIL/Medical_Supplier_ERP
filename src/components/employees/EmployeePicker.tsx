    ...
    <div className="relative" ref={rootRef} data-testid="employee-picker">
      ...
      {open ? (
        <div className="absolute z-30 mt-1 w-full min-w-[280px] overflow-hidden rounded-md border border-slate-200 bg-white shadow-xl">
          <div className="relative border-b border-slate-200 p-2"><Search className="absolute left-5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input ref={inputRef} className="h-10 w-full rounded border border-slate-200 pl-9 pr-3 text-sm outline-none focus:border-cyan-600" type="search" autoComplete="off" placeholder="Search name, ID, designation..." value={query} onChange={(event) => { setQuery(event.target.value); setActiveIndex(0); }} onKeyDown={(event) => {
