          ...
          <div className="grid gap-10 lg:grid-cols-[0.78fr_1.22fr]">
            <div>
              ...
              <div className="mt-7 grid gap-3">
                <div className="flex gap-3 border-b border-slate-200 pb-4"><MapPin className="mt-1 h-5 w-5 shrink-0 text-cyan-700" /><div><h2 className="text-sm font-bold text-slate-950">Office address</h2><p className="mt-1 text-sm leading-6 text-slate-600">{publicContact.addressLines.map((line) => <span className="block" key={line}>{line}</span>)}</p></div></div>
                <a className="flex gap-3 border-b border-slate-200 py-4 hover:text-blue-900" href={`tel:${publicContact.phoneHref}`}><Phone className="h-5 w-5 shrink-0 text-cyan-700" /><div><h2 className="text-sm font-bold text-slate-950">Phone</h2><p className="mt-1 text-sm text-slate-600">{publicContact.phone}</p></div></a>
                <a className="flex gap-3 border-b border-slate-200 py-4 hover:text-blue-900" href={`mailto:${publicContact.email}`}><Mail className="h-5 w-5 shrink-0 text-cyan-700" /><div><h2 className="text-sm font-bold text-slate-950">Email</h2><p className="mt-1 text-sm text-slate-600">{publicContact.email}</p></div></a>
                <a className="flex gap-3 py-4 hover:text-blue-900" href={`https://wa.me/${publicContact.phoneHref.replace("+", "")}`} target="_blank" rel="noreferrer"><MessageCircle className="h-5 w-5 shrink-0 text-cyan-700" /><div><h2 className="text-sm font-bold text-slate-950">WhatsApp</h2><p className="mt-1 text-sm text-slate-600">Open a business conversation</p></div></a>
              </div>
