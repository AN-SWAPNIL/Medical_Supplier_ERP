...
const metricIcons = [ShoppingCart, CircleDollarSign, Banknote, ReceiptText, Boxes, FileDown];
const metricTones = ["border-red-600", "border-cyan-600", "border-amber-500", "border-violet-500", "border-emerald-600", "border-blue-600"];
  ...
  return (
    <>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        <Panel title="Action queue" subtitle="Records needing the next operational step">
        ...
        <Panel title="Expiry awareness" subtitle="Expiry stays visible while FIFO controls issue sequence">
          <div className="divide-y divide-slate-100">
            {data.expiryAlerts.slice(0, 5).map((batch) => (
              <div className="flex items-center gap-3 px-4 py-3" key={batch.id}>
                ...
                <time className="text-xs font-bold text-amber-700">{batch.expiryDate}</time>
              </div>
            ))}
          </div>
        </Panel>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Panel title="Customer dues" subtitle="Highest current balances">
          <TableFrame>
        ...
        <Panel title="Recent activity" subtitle="Sales and collection posting">
        ...
        <Panel title="Operating expense pulse" subtitle="These costs remain separate from landed cost">
          <div className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4">
            {data.recentExpenses.slice(0, 4).map((expense) => (
              <div className="border-l-2 border-violet-500 pl-3" key={expense.id}>
