...
export default function EmployeesPage() {
  ...
  const openFromDirectory = (destination: "activity" | "field-team" | "report" | "access", selectedEmployeeId: string) => {
    if (destination === "report") {
      navigate(`/app/reports?view=marketing&employee=${selectedEmployeeId}&preset=month`);
      return;
    }
  ...
  return <>
    <PageHeader eyebrow="People and team management" title="Employees" subtitle="Employee profiles, login accounts, access, field activity and performance stay connected without creating a separate HR or payroll system." />
    <Segmented value={view} onChange={(next) => setView(next)} ariaLabel="Employee management views" options={([
      ...
      { value: "activity", label: "Activity & Performance" }
