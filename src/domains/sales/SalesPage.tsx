...
import type { Customer, Quotation, SalesInvoice, SalesOrder } from "../erp.types";
...
type ModalType = "customer" | "quotation" | "convert" | "delivery" | "collection" | "order-details" | "invoice" | "cancel-invoice" | null;
...
export default function SalesPage() {
  ...
  const [invoiceDeliveryIds, setInvoiceDeliveryIds] = useState<string[]>([]);
  ...
  const canDispatch = hasEffectivePermission(session?.user, "sales", "post") && hasEffectivePermission(session?.user, "inventory", "view");
  const canCollect = hasEffectivePermission(session?.user, "sales", "post");
  const canApproveInvoice = hasEffectivePermission(session?.user, "sales", "approve");
  const canCreateCustomer = hasEffectivePermission(session?.user, "customers", "create");
  ...
  const invoicesQuery = useQuery({ queryKey: ["sales", "invoices", session?.user.id], queryFn: salesService.invoices, enabled: canViewSales });
  ...
  const queries = [customersQuery, quotationsQuery, ordersQuery, deliveriesQuery, invoicesQuery, collectionsQuery, productsQuery];
  ...
  useEffect(() => {
    ...
    if (requestedAction === "quotation" && canCreateQuote) {
      setViewState("orders");
      setEditingQuotation(undefined);
      setModal("quotation");
    } else if (requestedAction === "collection" && canCollect) {
  ...
  }, [canCollect, canCreateQuote, params, setParams]);
  ...
  const invoiceSource = deliveries.find((delivery) => delivery.id === invoiceDeliveryIds[0]);
  const invoiceCandidates = invoiceSource ? deliveries.filter((delivery) => delivery.orderId === invoiceSource.orderId && !invoices.some((invoice) => invoice.status !== "Cancelled" && invoice.deliveryIds.includes(delivery.id))) : [];
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            {canViewReports && view !== "marketing" ? <Button icon={<FileBarChart className="h-4 w-4" />} onClick={() => navigate(`/app/reports/${view === "customers" ? "customer-ledger" : view === "orders" ? "sales-order-register" : view === "deliveries" ? "delivery-challan-register" : "collections"}`)}>Open Report</Button> : null}
      ...
      {view !== "marketing" ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-md border border-slate-200 bg-white p-4 shadow-sm"><span className="text-xs text-slate-500">Visible customers</span><strong className="mt-1 block text-2xl">{formatNumber(customers.length)}</strong><small className="text-slate-400">{role === "Sales Executive" ? "assigned to you" : "customer ledgers"}</small></div>
      ...
      {view === "customers" ? (
        <Panel title="Customer ledger" subtitle="All spreadsheet histories are combined into one searchable customer list with running balances." actions={<label><span className="sr-only">Open customer ledger</span><select className={inputClass + " min-w-56"} value="" onChange={(event) => setLedgerCustomer(customers.find((customer) => customer.id === event.target.value))}><option value="">Open detailed ledger...</option>{filteredCustomers.map((customer) => <option value={customer.id} key={customer.id}>{customer.name}</option>)}</select></label>}>
          <TableFrame>
                ...
                return (
                  <tr key={customer.id}>
                    ...
                    <td className="px-4 py-3"><div className="flex justify-end gap-1">
                      {canViewMarketing && canViewReports ? <Button variant="ghost" icon={<History className="h-4 w-4 text-cyan-700" />} onClick={() => navigate(`/app/reports/daily-marketing?preset=this-month&subject=${encodeURIComponent(`customer:${customer.id}`)}`)} aria-label={`Open marketing history for ${customer.name}`} title="Marketing history" /> : null}
      ...
      {view === "orders" ? (
        <>
          <div className="grid grid-cols-2 gap-2 rounded-md border border-slate-200 bg-white p-3 sm:grid-cols-5">
            {[["Quotation", FileText], ["Order", ShoppingCart], ["Delivery", Truck], ["Invoice", ReceiptText], ["Collection", Banknote]].map(([label, Icon], index) => <div className="relative flex items-center gap-2 rounded bg-slate-50 p-3" key={String(label)}><span className="grid h-8 w-8 place-items-center rounded bg-red-50 text-red-700"><Icon className="h-4 w-4" /></span><strong className="text-xs sm:text-sm">{label as string}</strong>{index < 4 ? <ArrowRight className="absolute -right-2 z-10 hidden h-4 w-4 text-slate-400 sm:block" /> : null}</div>)}
          </div>
          <Panel title="Quotations" subtitle="Digital and preprinted letterhead modes use this same record.">
            <TableFrame>
              <table className="min-w-[1020px] w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">{quotations.map((quote) => <tr key={quote.id}><td className="px-4 py-3"><strong>{quote.quotationNumber}</strong><small className="block text-slate-500">{quote.date} · {quote.validityDays} days</small></td><td className="max-w-72 px-4 py-3"><span className="block truncate font-semibold">{quote.customerName}</span></td><td className="px-4 py-3">{quote.lines.length}<small className="block text-slate-500">{formatNumber(quote.lines.reduce((sum, line) => sum + Number(line.quantity), 0))} units</small></td><td className="px-4 py-3 text-slate-600">{quote.paymentTerms}</td><td className="px-4 py-3 text-right font-bold">{formatCurrency(quote.total)}</td><td className="px-4 py-3"><StatusBadge status={quote.status} /></td><td className="px-4 py-3"><div className="flex justify-end gap-1">{canPrint ? <Button variant="ghost" icon={<Printer className="h-4 w-4" />} onClick={() => navigate("/app/print/quotation/" + quote.id)} aria-label="Print quotation" title="Print quotation" /> : null}{canEditQuote && !["Converted", "Rejected"].includes(quote.status) ? <Button variant="ghost" icon={<Pencil className="h-4 w-4" />} onClick={() => { setEditingQuotation(quote); setModal("quotation"); }} aria-label="Edit quotation" title="Edit quotation" /> : null}{canConvertQuote && quote.status !== "Converted" ? <Button variant="ghost" icon={<CheckCircle2 className="h-4 w-4 text-emerald-700" />} onClick={() => { setConverting(quote); setDeliveryInstruction(""); setModal("convert"); }}>Convert</Button> : null}{canDeleteQuote && ["Draft", "Rejected"].includes(quote.status) ? <Button variant="ghost" icon={<Trash2 className="h-4 w-4 text-red-600" />} onClick={() => setDeleteTarget({ type: "quotation", id: quote.id, label: quote.quotationNumber })} aria-label="Delete quotation" title="Delete quotation" /> : null}</div></td></tr>)}</tbody>
              </table>
            </TableFrame>
          </Panel>
          <Panel title="Sales orders" subtitle="Converted quotation lines carry forward without re-entry. Delivery posts stock-out; only an approved invoice posts receivable.">
            <TableFrame>
          ...
          <Panel title="Invoice register" subtitle="Invoices are created from posted delivery challans. Drafts do not affect customer due until approval.">
            <TableFrame><table className="min-w-[1050px] w-full text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase text-slate-500"><tr><th className="px-4 py-3">Invoice</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Delivery reference</th><th className="px-4 py-3 text-right">Value</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{invoices.map((invoice) => <tr key={invoice.id}><td className="px-4 py-3"><strong>{invoice.invoiceNumber}</strong><small className="block text-slate-500">{invoice.date}</small></td><td className="px-4 py-3 font-semibold">{invoice.customerName}</td><td className="px-4 py-3 text-slate-600">{invoice.deliveryIds.map((id) => deliveries.find((delivery) => delivery.id === id)?.challanNumber ?? id).join(", ")}</td><td className="px-4 py-3 text-right font-bold">{formatCurrency(invoice.total)}</td><td className="px-4 py-3"><StatusBadge status={invoice.status} /></td><td className="px-4 py-3"><div className="flex justify-end gap-1">{canPrint ? <Button variant="ghost" icon={<Printer className="h-4 w-4" />} onClick={() => navigate("/app/print/invoice/" + invoice.id)} title="Print invoice" aria-label="Print invoice" /> : null}{canApproveInvoice && invoice.status === "Draft" ? <Button variant="ghost" icon={<CheckCircle2 className="h-4 w-4 text-emerald-700" />} onClick={() => action.mutate({ run: () => salesService.approveInvoice(invoice.id), success: "Invoice approved and receivable posted" })}>Approve</Button> : null}{canApproveInvoice && invoice.status === "Approved" ? <Button variant="ghost" onClick={() => { setSelectedInvoice(invoice); setModal("cancel-invoice"); }}>Cancel</Button> : null}</div></td></tr>)}</tbody></table></TableFrame>
          </Panel>
        </>
      ) : null}
      ...
      {view === "deliveries" ? (
        <Panel title="Delivery challans" subtitle="Actual batch selection posts stock-out movements; a newer batch triggers a controlled FIFO warning.">
          <TableFrame>
            <table className="min-w-[1120px] w-full text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase text-slate-500"><tr><th className="px-4 py-3">Challan</th><th className="px-4 py-3">Order / Customer</th><th className="px-4 py-3">Products & Batches</th><th className="px-4 py-3">Receiver</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{deliveries.map((delivery) => { const linkedInvoice = invoices.find((invoice) => invoice.deliveryIds.includes(delivery.id) && invoice.status !== "Cancelled"); return <tr key={delivery.id}><td className="px-4 py-3"><strong>{delivery.challanNumber}</strong><small className="block text-slate-500">{delivery.date}</small></td><td className="px-4 py-3"><strong className="block">{orders.find((order) => order.id === delivery.orderId)?.orderNumber ?? delivery.orderId}</strong><small className="text-slate-500">{delivery.customerName}</small></td><td className="px-4 py-3">{delivery.lines.map((line) => <span className="mb-1 block text-xs" key={line.id}>{line.productCode} · {formatNumber(line.quantity)} · <b>{line.batchNumber}</b></span>)}</td><td className="px-4 py-3 text-slate-600">{delivery.receiverName ?? "-"}</td><td className="px-4 py-3"><StatusBadge status={delivery.status} />{linkedInvoice ? <small className="mt-1 block text-blue-700">{linkedInvoice.invoiceNumber}</small> : null}{delivery.overrideReason ? <small className="mt-1 block max-w-48 text-amber-700">Override: {delivery.overrideReason}</small> : null}</td><td className="px-4 py-3"><div className="flex justify-end gap-1"><Button variant="ghost" icon={<Printer className="h-4 w-4" />} onClick={() => navigate("/app/print/challan/" + delivery.id)} aria-label="Print challan" title="Print challan" />{canConvertQuote && !linkedInvoice ? <Button variant="ghost" icon={<ReceiptText className="h-4 w-4 text-blue-700" />} onClick={() => { setInvoiceDeliveryIds([delivery.id]); setModal("invoice"); }}>Create Invoice</Button> : null}</div></td></tr>; })}</tbody></table>
          </TableFrame>
        </Panel>
      ) : null}
      ...
      {view === "collections" ? (
        <div className="grid gap-4 xl:grid-cols-[1fr_360px]">
          <Panel title="Collection register" subtitle="Posting updates customer due and the selected cash or bank ledger.">
            <TableFrame>
              <table className="min-w-[950px] w-full text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase text-slate-500"><tr><th className="px-4 py-3">Receipt</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Mode / Reference</th><th className="px-4 py-3">Invoice / Order</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3 text-right">Print</th></tr></thead><tbody className="divide-y divide-slate-100">{collections.map((collection) => <tr key={collection.id}><td className="px-4 py-3"><strong>{collection.receiptNumber}</strong><small className="block text-slate-500">{collection.date}</small></td><td className="px-4 py-3 font-semibold">{collection.customerName}</td><td className="px-4 py-3 text-slate-600">{collection.paymentMode}<small className="block">{collection.referenceNumber ?? "-"}</small></td><td className="px-4 py-3 text-slate-600">{invoices.find((invoice) => invoice.id === collection.invoiceId)?.invoiceNumber ?? orders.find((order) => order.id === collection.orderId)?.orderNumber ?? "Customer ledger"}</td><td className="px-4 py-3 text-right font-bold text-emerald-700">{formatCurrency(collection.amount)}</td><td className="px-4 py-3 text-right"><Button variant="ghost" icon={<Printer className="h-4 w-4" />} onClick={() => navigate("/app/print/receipt/" + collection.id)} aria-label="Print receipt" title="Print receipt" /></td></tr>)}</tbody></table>
            </TableFrame>
          </Panel>
      ...
      {ledgerCustomer ? <Modal open title={`${ledgerCustomer.name} · Transaction Ledger`} subtitle="Approved invoices, collections and running due are generated from connected records; delivery value remains operational context." onClose={() => setLedgerCustomer(undefined)} width="max-w-6xl">{ledgerQuery.isLoading ? <LoadingBlock label="Building customer ledger" /> : ledgerQuery.isError || !ledgerQuery.data ? <ErrorBlock error={ledgerQuery.error} /> : <div className="grid gap-4"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><div className="rounded-md border border-slate-200 p-3"><span className="text-xs text-slate-500">Current Due</span><strong className="mt-1 block text-xl text-rose-700">{formatCurrency(ledgerQuery.data.currentDue)}</strong></div><div className="rounded-md border border-slate-200 p-3"><span className="text-xs text-slate-500">Invoiced Sales</span><strong className="mt-1 block text-xl">{formatCurrency(ledgerQuery.data.invoicedSales)}</strong></div><div className="rounded-md border border-slate-200 p-3"><span className="text-xs text-slate-500">Delivered Value</span><strong className="mt-1 block text-xl">{formatCurrency(ledgerQuery.data.deliveredSales)}</strong></div><div className="rounded-md border border-slate-200 p-3"><span className="text-xs text-slate-500">Collections</span><strong className="mt-1 block text-xl text-emerald-700">{formatCurrency(ledgerQuery.data.collected)}</strong></div><div className="rounded-md border border-slate-200 p-3"><span className="text-xs text-slate-500">Terms</span><strong className="mt-1 block text-sm">{ledgerQuery.data.customer.paymentTerms}</strong><small className="text-slate-500">{ledgerQuery.data.customer.phone}</small></div></div><div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600"><BookOpenText className="mr-2 inline h-4 w-4 text-cyan-700" />{ledgerQuery.data.customer.address} · Assigned territory {ledgerQuery.data.customer.territory}</div><TableFrame><table className="min-w-[900px] w-full text-left text-sm"><thead className="bg-slate-50 text-[11px] uppercase text-slate-500"><tr><th className="px-3 py-2">Date</th><th className="px-3 py-2">Type</th><th className="px-3 py-2">Reference</th><th className="px-3 py-2 text-right">Debit / Sale</th><th className="px-3 py-2 text-right">Credit / Collection</th><th className="px-3 py-2 text-right">Running Due</th><th className="px-3 py-2">Remarks</th></tr></thead><tbody className="divide-y divide-slate-100">{ledgerQuery.data.entries.map((entry) => <tr key={entry.id}><td className="px-3 py-3 text-slate-600">{entry.date}</td><td className="px-3 py-3"><StatusBadge status={entry.type} /></td><td className="px-3 py-3 font-semibold">{entry.reference}</td><td className="px-3 py-3 text-right">{Number(entry.debit) ? formatCurrency(entry.debit) : "-"}</td><td className="px-3 py-3 text-right text-emerald-700">{Number(entry.credit) ? formatCurrency(entry.credit) : "-"}</td><td className="px-3 py-3 text-right font-bold text-rose-700">{formatCurrency(entry.runningDue)}</td><td className="max-w-72 px-3 py-3 text-slate-600">{entry.remarks}</td></tr>)}</tbody></table></TableFrame></div>}</Modal> : null}
      {modal === "customer" ? <Modal open title={editingCustomer ? "Edit customer" : "Add customer"} subtitle="Each customer has one profile and one running sales and collection ledger." onClose={() => { setModal(null); setEditingCustomer(undefined); }}><CustomerForm customer={editingCustomer} busy={action.isPending} onSubmit={(payload) => action.mutate({ run: () => editingCustomer ? salesService.updateCustomer(editingCustomer.id, payload) : salesService.createCustomer(payload), success: editingCustomer ? "Customer updated" : "Customer created" })} /></Modal> : null}
      {modal === "quotation" ? <Modal open title={editingQuotation ? "Edit quotation" : "New quotation"} subtitle="Choose product variants; conversion carries every line into the sales order." onClose={() => { setModal(null); setEditingQuotation(undefined); setQuotationContext(undefined); }} width="max-w-5xl"><QuotationForm quotation={editingQuotation} initialCustomerId={quotationContext?.customerId} leadId={quotationContext?.leadId} customers={customers} products={products} canViewProfit={hasCapability(session?.user, "view_profit")} busy={action.isPending} onSubmit={(payload) => action.mutate({ run: () => editingQuotation ? salesService.updateQuotation(editingQuotation.id, payload) : salesService.createQuotation(payload), success: editingQuotation ? "Quotation updated" : "Quotation created" })} /></Modal> : null}
      ...
      {modal === "collection" ? <Modal open title="Post customer collection" subtitle="Link the receipt to an approved invoice when available; the payment credits customer due and the selected cash or bank account." onClose={() => { setModal(null); setCollectionCustomerId(undefined); }}><CollectionForm initialCustomerId={collectionCustomerId} customers={customers} orders={orders} invoices={invoices} accounts={accountsQuery.data ?? []} busy={action.isPending} onSubmit={(payload) => action.mutate({ run: () => salesService.createCollection(payload), success: "Collection posted and dues updated" })} /></Modal> : null}
      {modal === "invoice" ? <Modal open title="Create invoice from delivered goods" subtitle="Select eligible challans from the same order. The draft copies delivered lines exactly; receivable posts only after approval." onClose={() => { setModal(null); setInvoiceDeliveryIds([]); }}><form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); if (invoiceDeliveryIds.length) action.mutate({ run: () => salesService.createInvoice({ deliveryIds: invoiceDeliveryIds, date: new Date().toISOString().slice(0, 10) }), success: "Draft invoice created" }); }}><div className="rounded-md border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950"><strong>{invoiceSource?.customerName}</strong><span className="mt-1 block">{orders.find((order) => order.id === invoiceSource?.orderId)?.orderNumber}</span></div><fieldset className="grid gap-2"><legend className={labelClass}>Delivered challans to invoice</legend>{invoiceCandidates.map((delivery) => <label className="flex items-center gap-3 rounded-md border border-slate-200 p-3 text-sm" key={delivery.id}><input type="checkbox" checked={invoiceDeliveryIds.includes(delivery.id)} onChange={(event) => setInvoiceDeliveryIds((current) => event.target.checked ? [...new Set([...current, delivery.id])] : current.filter((id) => id !== delivery.id))} /><span className="flex-1"><strong className="block">{delivery.challanNumber}</strong><small className="text-slate-500">{delivery.date} · {formatNumber(delivery.lines.reduce((sum, line) => sum + Number(line.quantity), 0))} units</small></span></label>)}</fieldset><div className="flex justify-end"><Button type="submit" variant="primary" icon={<ReceiptText className="h-4 w-4" />} disabled={action.isPending || !invoiceDeliveryIds.length}>Create Draft Invoice</Button></div></form></Modal> : null}
      {modal === "cancel-invoice" && selectedInvoice ? <Modal open title={`Cancel ${selectedInvoice.invoiceNumber}`} subtitle="Cancellation reverses the posted receivable and requires an auditable reason." onClose={() => { setModal(null); setSelectedInvoice(undefined); setInvoiceReason(""); }}><form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); action.mutate({ run: () => salesService.cancelInvoice(selectedInvoice.id, invoiceReason), success: "Invoice cancelled and receivable reversed" }); }}><label><span className={labelClass}>Cancellation Reason</span><textarea className={textareaClass} minLength={5} required value={invoiceReason} onChange={(event) => setInvoiceReason(event.target.value)} /></label><div className="flex justify-end"><Button type="submit" variant="danger" disabled={action.isPending || invoiceReason.trim().length < 5}>Cancel Invoice</Button></div></form></Modal> : null}
